...
int handleLocCacheCall(unsigned int messageSize, char buffer[], rpcFunctionKey &key)
{
    ...
    while(messageSize > 0)
    {
        ...
        bufferP = b.extractUnsignedIntFromBuffer(bufferP, serverIDSize);
        char serverIDChar[serverIDSize];
        bufferP = b.extractCharArrayFromBuffer(bufferP, serverIDChar, serverIDSize);
        string serverID(serverIDChar);
        unsigned short port;
        bufferP = b.extractUnsignedShortFromBuffer(bufferP, port);
...
int sendExecuteRequest(int serverSocketFd,char* name, int* argTypes, void**args)
{
    ...
    switch(type)
    {
        case EXECUTE_SUCCESS:
        {
            ...
            replyMessageP = b.extractUnsignedIntFromBuffer(replyMessageP, nameSize);
            char nameChar[nameSize];

            replyMessageP = b.extractCharArrayFromBuffer(replyMessageP, nameChar, nameSize);
        ...
        {
            // extract error code
            b.extractIntFromBuffer(replyMessage, returnCode);
        }
        break;
        default:
...
int processLocResponse(string &serverID, unsigned short &port) {
    ...
    if (msgType == LOC_SUCCESS) {
        ...
        bufferPointer = b.extractUnsignedIntFromBuffer(bufferPointer, serverIDLength);

        char serverIDChar[serverIDLength];

        bufferPointer = b.extractCharArrayFromBuffer(bufferPointer, serverIDChar, serverIDLength);

        serverID.assign(serverIDChar);

        bufferPointer = b.extractUnsignedShortFromBuffer(bufferPointer, port);
        return 0;
        ...
        if(messageSize > 0)
        {
            ...
            b.extractIntFromBuffer(buffer, reasonCode);
        }
        return reasonCode;
    } else {
...
int rpcCacheCall(char* name, int* argTypes, void** args)
{
    ...
    {
        int errorCode;
        b.extractIntFromBuffer(buffer, errorCode);
    }
    else
    {
        return RECEIVE_INVALID_MESSAGE_TYPE;
