...
void handleRegisterRequest(Receiver &receiver, Sender &sender, char buffer[], unsigned int bufferSize, int serverFd) {
    ...
    try {
        ...
        bufferPointer = b.extractUnsignedIntFromBuffer(bufferPointer, nameLength);
        char nameChar[nameLength];

        bufferPointer = b.extractCharArrayFromBuffer(bufferPointer, nameChar, nameLength);
        string serverID(nameChar);
        bufferPointer = b.extractUnsignedShortFromBuffer(bufferPointer, port);
        bufferPointer = b.extractStringFromBuffer(bufferPointer, name);
        ...
        b.getArgTypes(bufferPointer, argTypes);
...
void handleLocCacheRequest(Receiver &receiver, Sender &sender, char buffer[], unsigned int bufferSize)
{
    try {
        ...
        bufferPointer = b.extractUnsignedIntFromBuffer(bufferPointer, nameSize);

        // getting function name
        char nameChar[nameSize];
        bufferPointer = b.extractCharArrayFromBuffer(bufferPointer, nameChar, nameSize);
